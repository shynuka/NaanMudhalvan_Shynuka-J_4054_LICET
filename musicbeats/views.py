...
def your_view(request):
    listen_later = YourModel.objects.filter(condition=True)
    return render(request, 'your_template.html', {'listen_later': listen_later})
...
def watchlater(request):
    if request.method == "POST":
        ...
        if Watchlater.objects.filter(user=user, video_id=video_id).exists():
    ...
    ids = wl.values_list('video_id', flat=True)
